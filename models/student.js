...
exports.search = function(filter, next){
  studentModel.find(filter, function(err,result){
    console.log(result);
    next(err,result);
  });
}

exports.editStudent = function(query, update, option, next){
  
  studentModel.findOneAndUpdate(query, update, option, function(err, result){
    console.log(result);
    next(err,result);
  });
}
